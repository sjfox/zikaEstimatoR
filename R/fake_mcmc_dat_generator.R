...
county_rnots <- county_r0_distributions %>% filter(month=="Aug") %>%
  unite(county_month, county, month, sep = "_") %>%
  t() %>%
  as_tibble()
...
for(intro in 1:(length(introductions)+1)){
  if(intro == (length(introductions)+1)){
    county_rnots_post[[4]] <- county_rnots
  } else{
...
county_fake_rnots <- county_rnots_post %>% purrr::map(gather) %>% bind_rows() %>%
  ...
  separate(key, into = c("county", "month"), sep = "_") %>%
  rename(rnots = value)
...
save(fake_alpha_dat, exp_secondary_cases, county_fake_rnots, file = "data_produced/fig1_data.rda")
